...
#+ by hcj for customize begin
TWD_CUSTOM_FOLDERS := twd/config
ifeq ($(strip $(TWD_ND)),yes)
TWD_CUSTOM_FOLDERS += twd/config/ND
endif
ifeq ($(strip $(TWD_2G)),yes)
TWD_CUSTOM_FOLDERS += twd/config/2G
endif
ifeq ($(strip $(TWD_1SIM)),yes)
TWD_CUSTOM_FOLDERS += twd/config/1SIM
endif
ifneq ($(TWD_PROJECT),)
ifneq ($(TWD_CUST_NAME),)
TWD_CUSTOM_FOLDERS += twd/custom/$(TWD_PROJECT)/$(TWD_CUST_NAME)
endif
endif
MTK_PROJECT_CONFIGS += $(call wildcard2,$(foreach c,$(addsuffix /ProjectConfig.mk,\
    $(addprefix ../../../,$(TWD_CUSTOM_FOLDERS))),$(call relative-path,$(c))))
#+ by hcj for customize end
